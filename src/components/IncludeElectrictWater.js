    ...
    const handleChoosePhoto = async (key) => {
        try {
            ...
            ImagePicker.openPicker(options).then(async (image) => {
                if (!!image) {
                    const res = await uploadRenterImage([image]);
                    dispatch({ field: key, value: res.Data });
                }
            });
        } catch (error) {
                ...
                <>
                    ...
                    <View style={[styles.formRow, styles.halfCol]}>
                        <Input
                            ...
                            onChangeText={(nextValue) =>
                                onChange("waterNumber", nextValue)
                            }
                            textContentType="none"
                            keyboardType="numeric"
                        />
                        ...
                        <>
                            <View style={[styles.formRow, styles.halfCol]}>
                                <Input
                                    ...
                                    onChangeText={(nextValue) =>
                                        onChange("electrictPrice", nextValue)
                                    }
                                    textContentType="none"
                                    keyboardType="numeric"
                                />
                    ...
                    <View style={[styles.formRow, styles.halfCol]}>
                        {state.electrictImage && (
                            <Image
                                source={{ uri: state.electrictImage[0].UrlIMG }}
                    ...
                    <View style={[styles.formRow, styles.halfCol]}>
                        {state.waterImage && (
                            <Image
                                source={{ uri: state.waterImage[0].UrlIMG }}
                                style={[styles.imagePreview]}
                            />
                        )}
                        <Button
                ...
                <>
                    <View style={[styles.formRow, styles.halfCol]}>
                        <Input
                            ...
                            onChangeText={(nextValue) =>
                                onChange("electrictPriceInclude", nextValue)
                            }
                            textContentType="none"
                            keyboardType="numeric"
                        />
                    ...
                    <View style={[styles.formRow, styles.halfCol]}>
                        <Input
                            ...
                            onChangeText={(nextValue) =>
                                onChange("waterPriceInclude", nextValue)
                            }
                            textContentType="none"
                            keyboardType="numeric"
                        />
                ...
                <>
                    <View style={[styles.formRow]}>
                        <Input
                            ...
                            label="Tiền trọn gói điện"
                            placeholder="0"
                            value={cf(state.electrictPriceInclude)}
                            onChangeText={(nextValue) =>
                                onChange("electrictPriceInclude", nextValue)
                            }
                            textContentType="none"

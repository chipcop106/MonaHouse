...
import { Icon, Input, Button, List } from '@ui-kitten/components';
...
const ModalizeAddMotel = forwardRef((props, ref) => {
	...
	const { setListMotels } = useContext(MotelContext);
	...
	const action_Submit = async () => {
		if(state.listMotel.length === 0){
			Alert.alert('Vui lòng nhập  phòng!!');
			return false;
		}
		if(!!state.listMotel.find((item) => item.name.trim().length === 0)){
			Alert.alert('Vui lòng nhập tên phòng!!', 'Hoặc vui lòng xoá các phòng không có tên');
			return false;
		}
		...
		try {
			...
			const res = await createMotelByName({
				listname: JSON.stringify(state.listMotel.map((s) => s.name)),
			});
			setspinner(false);
			...
			console.log('_Submit addMotel error', error)
			setspinner(false);
		}
	...
	const _renderItem = (item, index) => {
		return (
			<View style={styles.formRow} key={`${item.id}`}>
				<Input
					returnKeyType={'done'}
					...
					onBlur={({ nativeEvent }) =>
						_onChangeByIndex(index, nativeEvent.text)
					}
				/>
		...
		<Modalize ref={ref} {...props}>
			...
			<Button
				...
				<Text style={styles.btnSubmit}>
					Xác nhận thêm {state.listMotel.length} nhà
				</Text>
			</Button>

...
const calculateDiffPercentage = (cur, prev) =>
  cur === 0 && prev === 0
    ? 0
    : prev === 0
    ? 100
...
const calculateRatingRevenueNet = (revenue, net) =>
  net !== 0 ? ((net * 100) / revenue).toFixed(2) : 0;
const xAxisConfig = {
  valueFormatter: [
    "Jan",
    "Feb",
        ...
        <FlatList
          ...
          renderItem={({ item, index, seperators }) => (
            <View style={styles.roomWrap}>
              ...
              >
                {currencyFormat(item?.revenueNet ?? 0)}
              </Text>
            </View>
          )}
        />
    ...
    <>
      ...
      <View style={styles.container}>
            ...
            {state?.chartData?.length
              ? currencyFormat(
                  state.chartData.reduce(
                    (prev, cur) => prev + cur?.revenueNet ?? 0,
                    0
                  )
                )
              : 0}{" "}
        ...
        <View style={styles.chartWrap}>
          {state.isLoading ? (
            ...
            <CombinedChart
                  ...
                  dataSets: [
                    {
                      values: state.chartData.map((data, index) => ({
                        y: data?.revenue ?? 0,
                        x: index,
                        marker: `Doanh thu: ${currencyFormat(
                          data?.revenue ?? 0
                        )}`,
                      })),
                      label: "Doanh thu",
                  ...
                  dataSets: [
                    {
                      values: state.chartData.map((data, index) => ({
                        y: data?.revenueNet ?? 0,
                        x: index,
                        marker: `Lợi nhuận: ${currencyFormat(
                          data?.revenueNet ?? 0
                        )}`,
                      })),
                      label: "Lợi nhuận",
                      config: {
                        ...
                        lineWidth: 2,
                        drawCircles: true,
                        circleColor: processColor(color.redColor),
                        drawCircleHole: true,
                        circleRadius: 4,
                        color: processColor("rgba(255,61,113,0.35)"),
                        drawFilled: true,
                        fillGradient: {
                          colors: [
                            processColor("rgba(255,61,113,0.15)"),
                            processColor("rgba(255,61,113,0.05)"),
                          ],
                          positions: [0, 0.5],
                          angle: 90,
              ...
              highlights={[
                {
                  y: state?.chartData[state.selectedMonth]?.revenueNet ?? 0,
        ...
        <View style={{ ...styles.container }}>
          <View style={styles.wrapCard}>
            <View style={styles.summaryBox}>
              <View style={styles.box}>
                <Text style={styles.label}>Tỉ suất lợi nhuận năm</Text>
                <Text style={styles.value}>
                  {calculateRatingRevenueNet(
                    state.chartData.reduce(
              ...
              <View style={{ ...styles.box, borderRightWidth: 0 }}>
                <Text style={styles.label}>So với tháng trước</Text>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  {state.selectedMonth !== 0 &&
                  calculateDiffPercentage(
                    parseInt(
                      state?.chartData[state?.selectedMonth ?? 0]?.revenueNet ??
                        0
                    ),
                    parseInt(
          ...
          >
            <Text style={styles.title}>Lợi nhuận theo tháng</Text>
          </View>
...
export default SettingRevenueNetScreen;

      ...
      const res = await getReportOccupancyRatio({
        MotelID:
          state.activeMotel === 0
            ? 0
            : motelState.listMotels[state.activeMotel - 1].ID,
        Year: parseInt(yearOptions[state?.activeYear] ?? 0) || 0,
      });
      res.Code === 1 &&
        res?.Data?.length &&
        setChartData(
          res.Data.map((item) => ({
            ...
            rented: item?.Rented,
            empty: item?.Empty,
            marker: `Tỉ lệ ${item?.Y.toFixed(2) ?? 0}%`,
          }))
        );
    } catch (e) {
            ...
            <View style={styles.summaryBox}>
              <View style={styles.box}>
                ...
                <Text style={styles.value}>
                  {state?.chartData[state?.selectedMonth]?.total ?? 0}
                </Text>
              </View>
              <View style={styles.box}>
                <Text style={styles.label}>Đang thuê</Text>
                <Text style={styles.value}>
                  {state?.chartData[state?.selectedMonth]?.rented ?? 0}
                </Text>
              </View>
              <View style={styles.box}>
                <Text style={styles.label}>Còn trống</Text>
                <Text style={styles.value}>
                  {state?.chartData[state?.selectedMonth]?.empty ?? 0}
                </Text>
              </View>
              <View style={{ ...styles.box, borderRightWidth: 0 }}>
                <Text style={styles.label}>Tỉ lệ</Text>
                <Text style={styles.value}>
                  {state?.chartData[state?.selectedMonth]?.y.toFixed(2) ?? 0}%
                </Text>
              </View>
            </View>
          </View>
              ...
              <View style={styles.tabContentWrap}>
                {state.activeTab === 0 && (
                  <RenderListRoom
                    ...
                    data={state?.lastMonthRoom ?? []}
                  />
                )}
                {state.activeTab === 1 && (
                  <RenderListRoom
                    isLoading={state.roomLoading}
                    data={state?.thisMonthRoom ?? []}
                  />
                )}
                {state.activeTab === 2 && (
                  <RenderListRoom
                    isLoading={state.roomLoading}
                    data={state?.nextThreeMonthRoom ?? []}
                  />
                )}
              </View>
            </View>

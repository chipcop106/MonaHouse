...
import { StyleSheet, ScrollView,
    Text, View, RefreshControl, Alert, Animated, Easing
} from "react-native";
...
import sliderStyle, { sliderWidth, itemWidth, colors as sliderColors } from './sliderEntryOption/slidePackage'

const DotElement = props => {
    const {active , index} = props;
    const currentColor = !!active ? sliderColors.activeColor : sliderColors.inActiveColor;
    const targetColor = !!!active ? sliderColors.inActiveColor : sliderColors.activeColor;
    ...
    return (
        <View style={[sliderStyle.paginationDot]}>
            ...
            <Animated.View style={[ {}, animatedbgStyle]}>
                <View style={[sliderStyle.paginationDot, sliderStyle.paginationDotActive]}>
                    <View style={[sliderStyle.paginationDotInner, sliderStyle.paginationDotInnerActive]}></View>
                </View>
            </Animated.View>
        </View>
    )
...
const SettingPremiumPackageScreen = () => {
    ...
    const _renderItem = React.useMemo(() => ({item, index}) => {
        ...
        return (
            <View style={[styles.ptbWrap, isActive && shadowStyle]}>
                <View style={[styles.ptbHeader, {backgroundColor: typeColor}]}>
                    <Text style={{fontSize: 14, fontWeight: 'bold', color: "#fff", textTransform: "uppercase"}}>{item.title}</Text>
                </View>
                <View style={styles.ptbBody}>
                    <View style={styles.ptbPricebox}>
                        <Text style={[styles.ptbPrice_txt, { color: typeColor }]}>4,500,000</Text>
                        <Text style={{textTransform: "uppercase", color: "#7F7F7F"}}>vnd / tháng</Text>
                    </View>
                ...
                <View style={styles.ptbFooter}>
                    <Button style={[styles.btnSendPackage, {backgroundColor: typeColor}]}>
    ...
    return (<View style={styles.container}>
        ...
        >
            ...
            justifyContent: "center", alignItems: "center"}}><Loading /></View> : 
                (<><Carousel
                    layout={"default"}
                    ref={ref_carousel}
                    data={carouselItems}
                ...
                <Pagination
                    ...
                    dotElement={<DotElement active={true} carouselRef={ref_carousel} />}
                    inactiveDotElement={<DotElement active={false} carouselRef={ref_carousel}/>}
                />
            </>)}

...
const OtherRenterList = ({ renters, _deletePeople, _editPeople }) => {
  return (
    <>
      {renters.map((people) => {
        return (
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          licenseImages:
            renterimage.map((image) => ({
              ...image,
              UrlIMG: image.LinkIMG,
            })) || [],
          otherRenters:
            OtherRenters.map((renter) => ({
              ...renter,
              id: renter.ID,
              keyID: create_UUID(),
      ...
      <View style={styles.mainWrap}>
        ...
        <View style={{ marginBottom: 30 }}>
          {state.otherRenters?.length > 0 ? (
            <OtherRenterList
              renters={state?.otherRenters}
              _deletePeople={_deletePeople}
              _editPeople={_editPeople}
            />
          ) : (

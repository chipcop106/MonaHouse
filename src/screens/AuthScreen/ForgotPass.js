...
const ForgotPass = () => {
    ...
    return <KeyboardAvoidingView
            ...
            style={{ flex: 1 }}
        >
        <ScrollView contentContainerStyle={[{padding: 15 }]} removeClippedSubviews={false}>
    <Formik
        initialValues={PasswordData.empty()}
        validationSchema={PasswordSchema}
        onSubmit={_onFormSubmit}
    >
        {({handleSubmit}) => (<View style={styles.formContainer}>
            <View style={styles.formGroup}>
                <InputValidate
                    secureTextEntry={true}
                    label={ txtprops => <Text  {...txtprops} style={styles.label} >Mật khẩu:</Text>}
                    id='password'
                    // value={password}
            ...
            <View style={styles.formGroup}>
                <InputValidate
                    secureTextEntry={true}
                    label={ txtprops => <Text  {...txtprops} style={styles.label} >Xác nhận mật khẩu:</Text>}
                    id='confirmPassword'
                    // value={password}
                    // onChangeText={(value) => setPassword(value)}
                    style={styles.input}
